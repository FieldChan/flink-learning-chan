...
import com.cy.common.utils.KafkaConfigUtil;
...
public class OrderMatch {
    ...
    private static OutputTag<Order> bigOrderTag = new OutputTag<Order>("bigOrder"){};
    private static OutputTag<Order> smallOrderTag = new OutputTag<Order>("smallOrder"){};
    ...
    public static void main(String[] args) throws Exception {
        ...
        FlinkKafkaConsumerBase<String> consumerBigOrder =
                new FlinkKafkaConsumer<>("big_order",
        ...
        FlinkKafkaConsumerBase<String> consumerSmallOrder =
                new FlinkKafkaConsumer<>("small_order",
                        new SimpleStringSchema(),
                        KafkaConfigUtil.buildConsumerProps(KAFKA_CONSUMER_GROUP_ID))
                        .setStartFromGroupOffsets();
...
//                            ctx.timerService().registerProcessingTimeTimer(time);
                    ...
                    public void processElement2(Order smallOrder, Context ctx,
                        ...
                        if (bigOrder != null) {
                            ...
                            out.collect(Tuple2.of(smallOrder, null));
                            //处理订单多次来到的情况，删除上次的定时器
                            if (null!=timerState.value())
                                ctx.timerService().deleteEventTimeTimer(timerState.value());
                            long time = smallOrder.getCreateTime() + 60000;
                            timerState.update(time);
                            ctx.timerService().registerEventTimeTimer(time);
//                            ctx.timerService().registerProcessingTimeTimer(time);
                        }
